...
def full_network(params):
    ...
    if activation == 'linear':
        z, x_decode, encoder_weights, decoder_weights = linear_autoencoder(x, input_dim, latent_dim)
    else:
        z, x_decode, encoder_weights, decoder_weights = nonlinear_autoencoder(x, input_dim, latent_dim, params['widths'], activation=activation)
    
    if model_order == 1:
        dz = z_derivative(x, dx, encoder_weights, activation=activation)
        Theta = sindy_library_tf(z, latent_dim, poly_order, include_sine, include_constant)
    else:
        dz,ddz = z_derivative_order2(x, dx, ddx, encoder_weights, activation=activation)
    ...
    if model_order == 1:
        dx_decode = z_derivative(z, sindy_predict, decoder_weights, activation=activation)
    else:
        dx_decode,ddx_decode = z_derivative_order2(z, dz, sindy_predict, decoder_weights,
...
def define_loss(network, params):
    ...
    losses = {}
    losses['decoder'] = tf.reduce_mean((x - x_decode)**2) #+ tf.reduce_mean((network['dx'] - network['dx_decode'])**2)
    if params['model_order'] == 1:
...
def linear_autoencoder(x, input_dim, latent_dim):
    ...
    z,encoder_weights = build_network_layers(x, input_dim, latent_dim, [], None, 'encoder')
    x_decode,decoder_weights = build_network_layers(z, latent_dim, input_dim, [], None, 'decoder')

    return z, x_decode, encoder_weights,decoder_weights
...
def nonlinear_autoencoder(x, input_dim, latent_dim, widths, activation='elu'):
    ...
    z,encoder_weights = build_network_layers(x, input_dim, latent_dim, widths, activation_function, 'encoder')
    x_decode,decoder_weights = build_network_layers(z, latent_dim, input_dim, widths[::-1], activation_function, 'decoder')
...
def z_derivative(input, dx, weights, activation='elu'):
    """
    ...
    if activation == 'elu':
        for i in range(len(weights)-1):
            input = tf.matmul(input, weights[i])
...
def z_derivative_order2(input, dx, ddx, weights, activation='elu'):
    """
